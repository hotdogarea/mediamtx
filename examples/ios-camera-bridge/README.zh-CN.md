...
- OBS 声音不会直接进入直播 App 的麦克风回调。面板可检测 HLS 音轨和音频码率，并提供“静音 / 本机播放 / 外接内录”三种输出策略，但这三种都**不是软件麦克风注入**。真正的软件注入仍需单独实现音频解码、同步，并针对目标 App 的录音采集链适配。
...
3. “播了么推流助手”分为“画面 / 声音 / 授权”三个 Tab。首次使用先在“授权”页输入“推流助手”专用激活码；未激活时只禁用摄像头替换，不阻断宿主 App 的真实相机回调。授权通过后才可打开替换开关。画面页向普通用户只显示分辨率、码率、接收/送出帧率、当前/最高延迟，以及重试、卡顿、丢帧、保护黑帧等精简诊断数据。用户遇到问题时把这一页截图发给维护者即可。按住弹窗顶部标题可上下移动它。接收帧率是播放器实际拿到的画面速度，送出帧率是直播 App 实际收到的替换画面速度；它们不必严格相等。延迟仍是播放位置相对直播边缘的估算值，不是从 OBS 采集到观众看到画面的完整端到端延迟。
...
- **外接内录**：用于照片中那类带音频回录/环回能力的有线或 USB 转接设备。插件只有同时检测到外接音频输入和外接音频输出时才取消静音；拔掉设备或路线不完整时强制静音，避免意外改走扬声器。端口检测只能证明 iOS 看到了输入和输出，不能证明硬件内部确实完成环回，必须再用“语音备忘录”或直播 App 的私密预览录一段确认。

因此，在不插任何硬件时，本版插件**不能**把 OBS 声音直接交给抖音的麦克风。若 OBS 音轨显示“有音轨”但静音模式听不到声音是正常的；可先选“本机播放”或用 Safari 播放同一 HLS 地址验证源音轨。

音频页还分别显示喇叭和麦克风通道：

- **喇叭 / 声音输出**显示电脑声音是否正在发送和当前输出端口。无声时图标只有空心轮廓；在“外接内录”模式下，插件以 USB 回到麦克风后的真实电平从下往上填充图标。AVPlayer 的 HLS 音轨本身没有暴露可直接计量的输出 PCM，因此“本机播放”只能确认输出已打开，不能把码率伪装成音量。
