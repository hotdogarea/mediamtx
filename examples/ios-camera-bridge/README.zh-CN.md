...
- OBS 声音不会进入直播 App 的麦克风回调。面板现在只能检测 HLS 音轨和音频码率，**不能证明音轨里有实际声音，更不代表已经注入直播 App**。AVPlayer 保持静音，避免手机外放反馈到麦克风；真正的音频注入需要单独设计解码、同步与音频输入路径。
...
3. 面板上看“画面稳定 / 正在缓冲 / 重连”，以及**解码帧/秒、替换帧/秒、视频码率、分片下载吞吐、HLS 直播边缘落后量及本次峰值、重连次数、累计接收/替换帧、累计黑帧、OBS 音轨状态**。按住弹窗顶部标题可上下移动它。接收帧是从 HLS 实际取到的帧；替换帧是目标 App 实际收到的替换回调；它们不必严格相等。码率来自 AVPlayer 日志，无法读取时显示“暂无数据”。分片下载吞吐可能远高于视频码率，它不是 OBS 编码码率。HLS 落后量用播放位置与可跳转范围末端估算，**不是 OBS 采集到观众看到画面的端到端延迟**。这不是严格意义的 RTF。
...
建议连续测试 20–30 分钟，分别在开始、10 分钟、结束时记录面板截图；重点看 HLS 落后量是否持续上升，以及重连、卡顿、黑帧是否增加。30 帧/秒的短时截图只能说明当时帧率正常，不能证明长时间无积累延迟。若 OBS 音轨显示“有音轨”但手机里听不到，这是预期行为：本版只替换视频，音轨检测不等于声音输出或麦克风注入。可以先用 iPhone Safari 播放同一 HLS 地址并戴耳机验证 OBS 源本身是否有声。
