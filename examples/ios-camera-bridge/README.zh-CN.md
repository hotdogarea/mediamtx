...
- OBS 声音目前不会直接进入直播 App 的麦克风回调。1.0 面板移除了容易误解的“静音”，默认外放，并保留“硬件内录”兼容方式；这两种都**不是软件麦克风注入**。真正的“纯内录 / 内录加真人麦克风”仍需实现音频解码、同步、混音，并针对目标 App 的录音采集链适配。
- 可拖动的“播了么”浮动按钮及可按住标题上下移动的半透明设置弹窗属于**被注入 App 自己的窗口**，不是一个独立 App 跨 App 悬浮。官方 App 的窗口、输入链、完整性保护和版本差异，均可能让注入失败或画面不生效。
...
3. “播了么推流助手”标题会显示当前版本（1.0），并分为“画面 / 声音 / 授权”三个 Tab。首次使用先在“授权”页输入“推流助手”专用激活码；未激活时只禁用摄像头替换，不阻断宿主 App 的真实相机回调。授权通过后才可打开替换开关。画面页向普通用户只显示分辨率、码率、接收/送出帧率、当前/最高延迟，以及重试、卡顿、丢帧、保护黑帧等精简诊断数据。面板底部的“查看运行诊断日志”默认折叠，展开后可一键复制设备、宿主版本、画面、声音路线及最近运行事件；不记录激活码。日志保存在宿主 App 的缓存中，闪退后重新打开同一 App 仍可复制上一次记录。按住弹窗顶部标题可上下移动它。接收帧率是播放器实际拿到的画面速度，送出帧率是直播 App 实际收到的替换画面速度；它们不必严格相等。延迟仍是播放位置相对直播边缘的估算值，不是从 OBS 采集到观众看到画面的完整端到端延迟。
...
授权请求优先使用 `https://license.zltik.top`，产品类型固定为 `streaming_assistant`。服务器响应必须通过内置公钥的 Ed25519 签名校验，并同时绑定激活码、设备标识和产品类型；HTTPS 发生网络层故障时可回退旧 HTTP IP，但仍强制验签。成功状态保存在本机 Keychain，每 6 小时复验一次，临时断网最多宽限 72 小时且不能超过激活码自身到期时间。激活服务器的网络请求始终在摄像头回调之外执行，避免授权超时拖死直播采集线程。
...
- **硬件内录**：用于照片中那类带音频回录/环回能力的有线或 USB 转接设备。插件只有同时检测到外接音频输入和外接音频输出时才发送电脑声音；拔掉设备或路线不完整时不会意外改走扬声器。端口检测只能证明 iOS 看到了输入和输出，不能证明硬件内部确实完成环回，必须再用“语音备忘录”或直播 App 的私密预览录一段确认。

因此，在不插任何硬件时，本版插件**不能**把 OBS 声音直接交给抖音的麦克风。若 OBS 音轨显示“有音轨”但静音模式听不到声音是正常的；可先选“本机播放”或用 Safari 播放同一 HLS 地址验证源音轨。

音频页还分别显示喇叭和麦克风通道：

- **喇叭 / 声音输出**显示电脑声音是否正在发送和当前输出端口。无声时图标只有空心轮廓；在“硬件内录”模式下，插件以 USB 回到麦克风后的真实电平从下往上填充图标。AVPlayer 的 HLS 音轨本身没有暴露可直接计量的输出 PCM，因此“外放”只能确认输出已打开，不能把码率伪装成音量。
- **麦克风输入**显示直播 App 实际使用的输入端口，并从 `AVCaptureAudioDataOutput` 收到的 PCM 采样计算实时 dB/RMS 电平。支持常见浮点、8/16/24/32 位整数 PCM，包括 USB 设备常见的 24 位格式。无声时是空心图标；有声时绿色填充高度随真实音量变化。插入内录设备后，若这里显示 `USBAudio2.0` 且填充随声音跳动，才说明声音已经回到这一条麦克风采集链。
...
若看到“播了么”按钮却仍是真摄像头，先看“接收帧/秒”和“送出帧/秒”；接收有值但送出为 0，通常是目标 App 没走本插件挂钩的 `AVCaptureVideoDataOutput` 回调，或格式/预览另走一路。这是**针对 App 版本的适配工作**，不能靠修改流地址解决。若目标 App 根本不在 TrollFools 可选列表或注入后启动崩溃，当前路径到此为止；不提供“所有 iPhone 免越狱通用注入”的承诺。
